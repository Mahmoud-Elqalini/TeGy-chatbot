...
class Event(MainBase):
    ...
    event_source_id: Mapped[Optional[int]] = mapped_column(BigInteger, unique=True, nullable=True) # Used for sync tracking if needed
    organizer_id: Mapped[int] = mapped_column(BigInteger, nullable=False)
    name: Mapped[str] = mapped_column(String(100), nullable=False)
    description: Mapped[Optional[str]] = mapped_column(String(1000))
    category: Mapped[Optional[str]] = mapped_column(String(50))
    tags: Mapped[Optional[Union[dict, list]]] = mapped_column(JSONB)
    start_date: Mapped[DateTime] = mapped_column(DateTime(timezone=True), nullable=False)
    end_date: Mapped[DateTime] = mapped_column(DateTime(timezone=True), nullable=False)
    registration_deadline: Mapped[Optional[DateTime]] = mapped_column(DateTime(timezone=True))
    place: Mapped[Optional[str]] = mapped_column(String(500))
